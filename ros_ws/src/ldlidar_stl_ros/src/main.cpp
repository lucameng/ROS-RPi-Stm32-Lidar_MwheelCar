...
int main(int argc, char **argv) {
  ...
  if (cmd_port.Open(port_name)) {
    ROS_INFO("[ldrobot] open %s device %s is successful!", product_name.c_str(), port_name.c_str());
  }else {
    ROS_INFO("[ldrobot] failed to open %s device %s!", product_name.c_str(), port_name.c_str());
    exit(EXIT_FAILURE);
  }
